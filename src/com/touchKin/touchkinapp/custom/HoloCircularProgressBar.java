	...
	protected void onDraw(final Canvas canvas) {
		...
		if (mSlices != null) {
			for (PieSlice slice : mSlices) {
				mProgressColorPaint.setColor(slice.getColor());
				canvas.drawArc(mCircleBounds, currentAngle,
						getCurrentRotation(), false, mProgressColorPaint);
		...
		// canvas.rotate(45, mThumbPosX, mThumbPosY);
		// mSquareRect.left = mThumbPosX - 2 / 3;
		// mSquareRect.right = mThumbPosX + 220 / 3;
		// mSquareRect.top = mThumbPosY - 220 / 3;
		// mSquareRect.bottom = mThumbPosY + 220 / 3;
		// canvas.drawRect(mSquareRect, mThumbColorPaint);
